<script>
    import { onMount } from "svelte";
    import { goto } from "$app/navigation";
    import { fade, fly } from "svelte/transition";
    import { settings } from "$lib/stores/settings";
    import { API_URL } from "$lib/constants";

    let username = $state("");
    let email = $state("");
    let password = $state("");
    let confirmPassword = $state("");
    let error = $state("");
    let success = $state("");
    let loading = $state(false);
    let recaptchaToken = $state("");

    onMount(() => {
        // @ts-ignore
        if (window.grecaptcha) {
            // @ts-ignore
            window.grecaptcha.render("recaptcha-container", {
                sitekey: "6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI",
                callback: (token) => {
                    recaptchaToken = token;
                },
            });
        }
    });

    async function handleRegister() {
        if (password !== confirmPassword) {
            error = "Passwords do not match";
            return;
        }

        if (!recaptchaToken) {
            error = "Please complete the reCAPTCHA";
            return;
        }

        loading = true;
        error = "";
        success = "";

        try {
            const response = await fetch(`${API_URL}/api/register`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({
                    username,
                    email,
                    password,
                    recaptcha_token: recaptchaToken,
                }),
            });

            const data = await response.json();

            if (response.ok) {
                success = "Account initialized successfully!";
                setTimeout(() => goto("/login"), 1500);
            } else {
                error = data.msg || "Initialization failed";
                // @ts-ignore
                window.grecaptcha.reset();
                recaptchaToken = "";
            }
        } catch (e) {
            error = "Network link failure. Try again.";
        } finally {
            loading = false;
        }
    }
</script>

<div
    class="min-h-[calc(100vh-2.75rem)] flex items-center justify-center py-12 px-6 relative overflow-hidden mesh-gradient"
>
    <!-- Dynamic Glows -->
    <div
        class="absolute top-1/2 left-0 w-[600px] h-[600px] rounded-full blur-[150px] opacity-10 pointer-events-none"
        style="background: radial-gradient(circle, var(--primary), transparent)"
    ></div>

    <div
        class="max-w-md w-full relative z-10"
        in:fly={{ y: 20, duration: 800 }}
    >
        <div
            class="fd-card !p-10 !rounded-[3.5rem] shadow-2xl bg-fd-card/80 backdrop-blur-xl border-fd-primary/10"
        >
            <div class="text-center mb-10">
                <div
                    class="inline-flex items-center justify-center w-16 h-16 rounded-[1.5rem] bg-indigo-500 text-white mb-6 shadow-2xl shadow-indigo-500/30"
                >
                    <i class="bx bx-user-plus text-3xl"></i>
                </div>
                <h2
                    class="text-4xl font-[900] text-fd-foreground tracking-tighter"
                >
                    Initialize
                </h2>
                <p
                    class="mt-3 text-sm text-fd-muted font-bold tracking-tight opacity-70"
                >
                    Create your biological signature profile
                </p>
            </div>

            <form
                class="space-y-6"
                onsubmit={(e) => {
                    e.preventDefault();
                    handleRegister();
                }}
            >
                <div class="space-y-5">
                    <div class="space-y-2">
                        <label for="username" class="fd-label ml-1"
                            >Username</label
                        >
                        <div class="relative group">
                            <div
                                class="absolute inset-y-0 left-4 flex items-center pointer-events-none text-fd-muted group-focus-within:text-fd-primary transition-colors"
                            >
                                <i class="bx bx-at text-xl"></i>
                            </div>
                            <input
                                id="username"
                                type="text"
                                required
                                class="fd-input w-full !pl-12 !py-4 !rounded-2xl !bg-fd-secondary/30"
                                placeholder="unique_handle"
                                bind:value={username}
                            />
                        </div>
                    </div>

                    <div class="space-y-2">
                        <label for="email-address" class="fd-label ml-1"
                            >Network Email</label
                        >
                        <div class="relative group">
                            <div
                                class="absolute inset-y-0 left-4 flex items-center pointer-events-none text-fd-muted group-focus-within:text-fd-primary transition-colors"
                            >
                                <i class="bx bx-envelope text-xl"></i>
                            </div>
                            <input
                                id="email-address"
                                type="email"
                                required
                                class="fd-input w-full !pl-12 !py-4 !rounded-2xl !bg-fd-secondary/30"
                                placeholder="name@domain.com"
                                bind:value={email}
                            />
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label for="password" class="fd-label ml-1"
                                >Key</label
                            >
                            <input
                                id="password"
                                type="password"
                                required
                                class="fd-input w-full !py-4 !rounded-2xl !bg-fd-secondary/30"
                                placeholder="••••"
                                bind:value={password}
                            />
                        </div>
                        <div class="space-y-2">
                            <label for="confirm-password" class="fd-label ml-1"
                                >Sync</label
                            >
                            <input
                                id="confirm-password"
                                type="password"
                                required
                                class="fd-input w-full !py-4 !rounded-2xl !bg-fd-secondary/30"
                                placeholder="••••"
                                bind:value={confirmPassword}
                            />
                        </div>
                    </div>
                </div>

                <div
                    class="flex justify-center transform scale-90 origin-center py-2"
                >
                    <div
                        id="recaptcha-container"
                        class="rounded-xl overflow-hidden border border-fd-border/50"
                    ></div>
                </div>

                {#if error}
                    <div
                        class="text-red-500 text-xs font-black uppercase tracking-widest px-4 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-center animate-shake"
                    >
                        <i class="bx bx-error-circle mr-2"></i>
                        {error}
                    </div>
                {/if}

                {#if success}
                    <div
                        class="text-emerald-500 text-xs font-black uppercase tracking-widest px-4 py-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center"
                    >
                        <i class="bx bx-check-double mr-2"></i>
                        {success}
                    </div>
                {/if}

                <button
                    type="submit"
                    disabled={loading}
                    class="fd-btn-primary w-full !py-4.5 !rounded-2xl group relative overflow-hidden"
                >
                    <span class="relative z-10 flex items-center gap-3">
                        {#if loading}
                            <i class="bx bx-loader-alt animate-spin text-xl"
                            ></i>
                            Initializing...
                        {:else}
                            Create Signature
                            <i
                                class="bx bx-right-arrow-alt text-2xl group-hover:translate-x-2 transition-transform"
                            ></i>
                        {/if}
                    </span>
                    <div
                        class="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"
                    ></div>
                </button>
            </form>

            <div class="mt-8 pt-8 border-t border-fd-border/50 text-center">
                <p class="text-xs text-fd-muted font-bold tracking-tight">
                    Already part of the network?
                    <a
                        href="/login"
                        class="text-fd-foreground hover:text-fd-primary transition-colors font-black border-b border-fd-primary"
                    >
                        Access Portal
                    </a>
                </p>
            </div>
        </div>
    </div>
</div>
